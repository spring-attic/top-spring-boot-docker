...
With Spring Boot 2.3 you have the option of building an image from Maven or Gradle directly with Spring Boot. As long as you are already building a Spring Boot jar file, you only need to call the plugin directly. With https://docs.spring.io/spring-boot/docs/current/maven-plugin/reference/html/#build-image[Maven]:

```
$ ./mvnw spring-boot:build-image
```

and with https://docs.spring.io/spring-boot/docs/current/gradle-plugin/reference/html/#build-image[Gradle]
...
NOTE: The Spring Boot Maven and Gradle plugins use buildpacks in exactly the same way that the `pack` CLI does in the examples below. The main difference is that the plugins use `docker` to run the builds, whereas `pack` doesn't need to. The resulting images are identical given the same inputs.
